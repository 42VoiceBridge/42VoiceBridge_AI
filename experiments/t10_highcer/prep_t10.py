...
하는 일
  1. t10_manifest.json에 적힌 4개 wav를 찾아
  2. 16kHz / 모노 / 16bit 로 변환해서 t10_16k/ 에 저장
  3. 변환 결과를 검증하고 t10_16k/manifest.json 에 실측값을 덧붙여 다시 쓴다
...
실행
    ...
    python3 experiments/t10/prep_t10.py
...
ROOT = os.path.dirname(os.path.abspath(__file__))          # experiments/t10/
# manifest 의 src 는 프로젝트 루트(sw_challenge/) 기준 상대경로다. 2026-09-18 폴더 재편 후
# 스크립트가 experiments/t10/ 로 옮겨졌으므로 원본 색인은 두 단계 위에서 한다.
PROJECT = os.path.dirname(os.path.dirname(ROOT))
MANIFEST = os.path.join(ROOT, "t10_manifest.json")
OUTDIR = os.path.join(ROOT, "t10_16k")
TARGET_RATE = 16000
...
def main():
    if not os.path.exists(MANIFEST):
        sys.exit("t10_manifest.json 이 없다. 이 스크립트와 같은 폴더에 두고 다시 실행한다.")
