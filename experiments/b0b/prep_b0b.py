...
# -*- coding: utf-8 -*-
"""
B0 후속 측정용 전처리 (자유발화 03 · 대화체 06-01) — 로컬(맥)에서 실행한다.

하는 일
  1. b0b_manifest.json에 적힌 5개 wav를 찾아
  2. 16kHz / 모노 / 16bit 로 변환해서 b0b_16k/ 에 저장
  3. 변환 결과를 검증하고 b0b_16k/manifest.json 에 실측값을 덧붙여 다시 쓴다
...
실행
    ...
    python3 experiments/b0b/prep_b0b.py
...
ROOT = os.path.dirname(os.path.abspath(__file__))          # experiments/b0b/
# manifest 의 src 는 프로젝트 루트(sw_challenge/) 기준 상대경로다. 2026-09-18 폴더 재편 후
# 스크립트가 experiments/b0b/ 로 옮겨졌으므로 원본 색인은 두 단계 위에서 한다.
PROJECT = os.path.dirname(os.path.dirname(ROOT))
MANIFEST = os.path.join(ROOT, "b0b_manifest.json")
OUTDIR = os.path.join(ROOT, "b0b_16k")
TARGET_RATE = 16000
...
def main():
    if not os.path.exists(MANIFEST):
        sys.exit("b0b_manifest.json 이 없다. 이 스크립트와 같은 폴더에 두고 다시 실행한다.")

    man = json.load(open(MANIFEST, encoding="utf-8"))
    items = man["items"]
    print("대상 %d 파일 (자유발화 3 + 대화체 2)" % len(items))
